...
export const parseWorkbook = (buffer: Buffer, draftId: string, supplier: string = 'cellar'): ParsedWorkbook => {
  ...
  rows.slice(1).forEach((row, rowIndex) => {
    ...
    products.push({
      ...
      descriptionHtml: '',
      brand: textValue(valueAt(row, StandardCol.BRAND)),
      country: '',
      region: '',
      productType: textValue(valueAt(row, StandardCol.PRODUCT_TYPE)),
      supplierType,
      sourcePlatform: '',
      abv: '',
      containerType: textValue(valueAt(row, StandardCol.CONTAINER_TYPE)),
      style: '',
