...
- `src/App.css` and `src/workspace.css` contain the application styling. The workspace uses a flex-column shell so the product table and details editor expand full-height to the browser viewport, with internal scrolling for content overflow. On narrow screens the layout collapses to a single column. A warm cellar-shop palette with amber radial lighting glows is applied in both light and dark themes.
...
The local suggested sale price maps to the Shopify variant retail `price`; local unit price maps to the Shopify inventory-item `cost` ("Cost per item"). The app `brand` maps to the Shopify product `vendor`, the app `productType` maps to the Shopify `productType`, and Shopify `tags` are derived from a non-empty subset of `[brand, productType, country]`.
